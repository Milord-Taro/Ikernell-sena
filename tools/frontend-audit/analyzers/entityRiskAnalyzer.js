// Deriva el "slug" de ruta a partir del primer endpoint de la entidad.
// Ej: '/api/tipos-error' -> 'tipos-error', '/api/usuarios/{idUsuario}' -> 'usuarios'
function deriveSlug(entityEndpoints) {
...
export function analyzeEntityRisk(backendEntities, coverageRows, frontendRoutes) {
    ...
    for (const entity of backendEntities) {
        ...
        const slug = deriveSlug(entity.endpoints);
        const hasRoute = slug
